...
pub fn deserialize_merkle_proof(bytes: &[u8]) -> Result<MerkleProof> {
    msg!("deserialize_merkle_proof: Starting with {} bytes", bytes.len());
    msg!("deserialize_merkle_proof: First 16 bytes: {:?}", &bytes[..bytes.len().min(16)]);
    ...
    msg!("deserialize_merkle_proof: First byte 0x{:02x} = field {} wire type {}", 
        first_byte, field_number, wire_type);
    ...
    match result {
        ...
        Err(e) => {
            msg!("deserialize_merkle_proof: Direct MerkleProof decode failed: {:?}", e);
...
fn convert_proof_ops_to_merkle_proof(bytes: &[u8]) -> Result<MerkleProof> {
    ...
    let merkle_proof = <MerkleProof as Protobuf<RawMerkleProof>>::decode_vec(&encoded)
        .map_err(|e| {
            msg!("convert_proof_ops_to_merkle_proof: Conversion failed: {:?}", e);
